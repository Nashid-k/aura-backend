...
router.get('/nudge', async (request, response) => {
  try {
    const { habitCards, summary } = await getHabitContext(request.user._id);
    ...
    return response.json({
      nudge: chatCompletion.choices[0]?.message?.content || '',
    });
  } catch (error) {
    console.error('Groq nudge error:', error.message);
