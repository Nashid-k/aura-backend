...
import Achievement from '../modules/insights/models/Achievement.js';
 ...
 */
export async function checkAndAward(userId, habitStats) {
  const newBadges = [];
 ...
 */
export async function awardAchievement(userId, { type, habitId, habitTitle, metadata }) {
  const def = BADGE_DEFS[type] || {};
  const badge = await Achievement.create({ user: userId, type, habitId, habitTitle, metadata });
  return {
    _id: badge._id,
